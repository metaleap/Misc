...
void outBufAppend(Str str) {
    Ed.out_buf.at = realloc(Ed.out_buf.at, Ed.out_buf.len + str.len);
    if (Ed.out_buf.at == NULL)
        fail("out of memory");
    memcpy(&Ed.out_buf.at[Ed.out_buf.len], str.at, str.len);
    Ed.out_buf.len += str.len;
}
